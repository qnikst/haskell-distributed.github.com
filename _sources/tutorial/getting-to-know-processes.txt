...
system process or on a remote host! Instead, you should use the :api-dp:`exit` and :api-dp:`kill`
...
:api-dp:`die` primitive simplifies doing so without worrying about the expected type for the
...
:api-dp:`ProcessExitException` directly in the calling thread, thus terminating it without delay.
...
is also terminated since :distributed-process:`link <Control-Distributed-Process.html?v:link>`
will *link the lifetime of two processes together* regardless
of exit reasons.
...
:hackage-pkg:`distributed-process-platform` provides an alternate function ``linkOnFailure``,
...
The :api-dp:`getProcessInfo` function provides a means for us to obtain information about a running
process. The `:api-dp:`ProcessInfo` type it returns contains the local node id and a list of
...
could :api-dp:`getProcessId` and :api-dp:`getNodeId` respectively.
